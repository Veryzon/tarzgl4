...
ZGL afxError checkForResizeDxgi(sglDpu* dpu, afxDrawOutput dout);

_ZGL afxError setObjectsLockedDxgi(sglDpu* dpu, afxDrawOutput dout, afxBool locked)
{
...
_ZGL afxError swapDxgi(sglDpu* dpu, afxDrawOutput dout)
{
    afxError err;
...
_ZGL afxError postSubBufferDxgi(sglDpu* dpu, afxDrawOutput dout, afxRect const* rc)
{
    afxError err;
    glVmt const* gl = &dpu->gl;
    ...
    {
        RECT rect = { (LONG)(rc->x), (LONG)(dout->m.whd[1] - rc->y - rc->h), (LONG)(rc->x + rc->w), (LONG)(dout->m.whd[1] - rc->y) };
...
_ZGL afxError bindTexImageDxgi(sglDpu* dpu, afxDrawOutput dout, afxRaster texture, GLint buffer)
{
    afxError err;
    afxDrawDevice ddev = AfxGetParent(dout);
...
_ZGL afxError createSwapchainDxgi(sglDpu* dpu, afxDrawOutput dout);

_ZGL afxError checkForResizeDxgi(sglDpu* dpu, afxDrawOutput dout)
{
    afxError err = NIL;
...
_ZGL afxError createSwapchainDxgi(sglDpu* dpu, afxDrawOutput dout)
{
    ...
    else
    {
        ...
        if (SUCCEEDED(result))
        {
            ...
            swapChainDesc.Width = (UINT)(dout->m.whd[0]);
            swapChainDesc.Height = (UINT)(dout->m.whd[1]);
            swapChainDesc.Format = dout->dxgi.mSwapChainFormat;
        ...
        else
        {
            ...
            swapChainDesc.BufferDesc.Width = (UINT)(dout->m.whd[0]);
            swapChainDesc.BufferDesc.Height = (UINT)(dout->m.whd[1]);
