...
{
    glVmt* gl = &(((afxDrawSystem)AfxGetParent(&pip->obj))->vmt);
#if 0
    ...
    for (afxInt i = 0; i < cnt; i++)
    {
        ...
        res.binding = binding;
        res.resType = avxShaderResType_CONSTANT_BUFFER;
        res.set = 0;
        res.location = i;
        res.arraySize = 0;
        res.vecSize = 0;
    ...
    for (GLuint i = 0; i < (GLuint)cnt; i++)
    {
        ...
        if (blockIdx == -1)
        {
            ...
            case GL_SAMPLER_1D:
                ...
                res.resType = avxShaderResType_COMBINED_IMAGE_SAMPLER;
                valid = TRUE;
                break;
            case GL_SAMPLER_2D:
                gl->GetActiveUniformName(gpuHandle, i, sizeof(res.name), NIL, res.name); _SglThrowErrorOccuried();
...
_ZGL afxError _DpuBindAndSyncPip(sglDpu* dpu, afxBool bind, afxBool sync, avxPipeline pip)
{
    //AfxEntry("pip=%p", pip);
    ...
    else
    {
        ...
        sglUpdateFlags devUpdReq = (pip->updFlags & ZGL_UPD_FLAG_DEVICE);
        afxBool bound = FALSE;
...
_ZGL afxError _SglPipCtor(avxPipeline pip, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &pip, afxFcc_PIP);
