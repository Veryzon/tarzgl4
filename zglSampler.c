...
_ZGL afxError _DpuBindAndSyncSamp(sglDpu* dpu, afxNat glUnit, avxSampler samp)
{
    //AfxEntry("smp=%p", smp);
    ...
    else
    {
        ...
        sglUpdateFlags devUpdReq = (samp->updFlags & ZGL_UPD_FLAG_DEVICE);
...
_ZGL afxError _SglSampDtor(avxSampler samp)
{
    ...
    if (samp->glHandle)
    {
        afxDrawContext dctx = AfxGetParent(samp);
...
_ZGL afxError _SglSampCtor(avxSampler samp, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &samp, afxFcc_SAMP);
