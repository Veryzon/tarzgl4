...
_ZGL afxError _DpuBindAndSyncSamp(zglDpu* dpu, afxUnit glUnit, avxSampler samp)
{
    ...
    else
    {
        ...
        if ((!glHandle) || (devUpdReq & ZGL_UPD_FLAG_DEVICE_INST))
        {
            ...
            GLenum magF = SglToGlTexelFilterMode(samp->m.cfg.base.magnify);
            GLenum minF = SglToGlTexelFilterModeMipmapped(samp->m.cfg.base.minify, samp->m.cfg.base.mipify);
...
_ZGL afxError _SglSampDtor(avxSampler samp)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &samp, afxFcc_SAMP);
    ...
    if (samp->glHandle)
    {
        afxDrawContext dctx = AfxGetProvider(samp);
        _SglDctxEnqueueDeletion(dctx, 0, GL_SAMPLER, (afxSize)samp->glHandle);
        samp->glHandle = 0;
    }
...
_ZGL afxError _SglSampCtor(avxSampler samp, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &samp, afxFcc_SAMP);

    if (_AvxSampStdImplementation.ctor(samp, args, invokeNo)) AfxThrowError();
    else
    {
        samp->glHandle = 0;
