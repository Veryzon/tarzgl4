...
_ZGL afxError _SglCmdbResetCb(avxCmdb cmdb)
{
    ...
    AfxChainForEveryLinkageB2F(&cmdb->commands, _zglCmd, script, cmdHdr)
    {
        ...
        {
            //_zglCmdBeginSynthesis const *cmd = AFX_REBASE(cmdHdr, _zglCmdBeginSynthesis, cmd);
        }
    }
    ...
    while (1)
    {
        ...
        {
            _zglCmd *cmd = AFX_REBASE(first, _zglCmd, script);
...
ZGL afxError _SglCmdbEndCb(avxCmdb cmdb); // no zglDrawCommands.c
...
_ZGL afxError _SglCmdbCtor(avxCmdb cmdb, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &cmdb, afxFcc_CMDB);
