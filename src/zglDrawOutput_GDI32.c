...
_ZGL afxUnit _SglDoutIsSuspended(afxDrawOutput dout)
{
...
_ZGL afxUnit _SglDoutSuspendFunction(afxDrawOutput dout)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dout, afxFcc_DOUT);
...
_ZGL afxUnit _SglDoutResumeFunction(afxDrawOutput dout)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dout, afxFcc_DOUT);
    AfxEnterSlockExclusive(&dout->m.suspendSlock);
...
_ZGL afxError _SglDoutPresentGdi(afxDrawOutput dout, afxUnit bufIdx, afxUnit queCnt, afxDrawQueue queues[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dout, afxFcc_DOUT);
    ...
    else
    {
        ...
        if (buf->glHandle)
        {
            if (!dout->swapFboReady[bufIdx])
            {
                ...
                {
                    gl->GenFramebuffers(1, &dout->swapFbo[bufIdx]); _SglThrowErrorOccuried();
                    gl->BindFramebuffer(GL_READ_FRAMEBUFFER, dout->swapFbo[bufIdx]); _SglThrowErrorOccuried();
                    AfxAssert(gl->IsFramebuffer(dout->swapFbo[bufIdx]));
                }
                else
                {
                    AfxAssert(gl->IsFramebuffer(dout->swapFbo[bufIdx]));
            ...
            gl->BlitFramebuffer(0, 0, whd.w, whd.h, x, y, w, h, GL_COLOR_BUFFER_BIT, GL_NEAREST); _SglThrowErrorOccuried();
...
_ZGL afxBool _SglDouVmtGetIddCb(afxDrawOutput dout, afxUnit code, void* dst)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dout, afxFcc_DOUT);
...
_ZGL afxError _SglRelinkDoutCb(afxDrawDevice ddev, afxDrawContext dctx, afxUnit cnt, afxDrawOutput douts[])
{
    afxError err = AFX_ERR_NONE;
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        ...
        if (dout->hSwapRC)
        {
            ...
            for (afxUnit i = 0; i < dout->m.bufCnt; i++)
            {
                if (dout->swapFbo[i])
                {
                    AfxAssert(gl->IsFramebuffer(dout->swapFbo[i]));
                    gl->DeleteFramebuffers(1, &dout->swapFbo[i]); _SglThrowErrorOccuried();
                    dout->swapFbo[i] = NIL;
...
_ZGL afxError _SglDoutCtorCb(afxDrawOutput dout, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dout, afxFcc_DOUT);
    ...
    else
    {
        ...
        dout->swapFbo = AfxAllocate(dout->m.bufCnt, sizeof(dout->swapFbo[0]), 0, AfxHere());
        dout->swapFboReady = AfxAllocate(dout->m.bufCnt, sizeof(dout->swapFboReady[0]), 0, AfxHere());
...
_ZGL afxError _SglDoutDtorCb(afxDrawOutput dout)
{
    ...
    if (dout->swapFbo)
        AfxDeallocate(dout->swapFbo), dout->swapFbo = NIL;

    if (dout->swapFboReady)
        AfxDeallocate(dout->swapFboReady), dout->swapFboReady = NIL;
