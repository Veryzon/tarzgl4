...
#include "../../icd_wshell4d/src/AuxOverWin32.h"
...
_QOW afxError _ZglDoutPresent_WGL(afxDrawQueue dque, avxPresentation* ctrl)
{
    ...
    if (buf->glHandle)
    {
        ...
        afxInt x = (dout->m.presentTransform & avxVideoTransform_FLIP_H) ? rc.w : 0;
        afxInt y = (dout->m.presentTransform & avxVideoTransform_FLIP_V) ? rc.h : 0;
        afxInt w = (dout->m.presentTransform & avxVideoTransform_FLIP_H) ? 0 : rc.w;
        afxInt h = (dout->m.presentTransform & avxVideoTransform_FLIP_V) ? 0 : rc.h;
...
#if 0
_QOW afxError _DpuPresentDout_BlitCanvToGdi(zglDpu* dpu, afxSurface dout, afxUnit outBufIdx)
{
    afxError err = AFX_ERR_NONE;
    ...
    afxWarp whd;
    AvxGetSurfaceArea(dout, &whd, NIL);
#if 0
    int a = 0;
    HDC dc = dout->hDC;
...
_QOW afxError _DpuPresentDout_BlitSurfToGdi(zglDpu* dpu, afxSurface dout, afxUnit outBufIdx)
{
    afxError err = AFX_ERR_NONE;
    ...
    avxRaster surf;
    AvxGetSurfaceBuffer(dout, outBufIdx, &surf);
...
_QOW afxError _ZglRelinkDoutCb_WGL(afxSurface dout)
{
    ...
    if (dout->wgl.hSwapRC)
    {
        afxDrawSystem currDsys = AvxGetSurfaceSystem(dout);
    ...
    else
    {
        ...
        else
        {
            ...
            if ((dout->m.presentMode & avxPresentFlag_VSYNC))
