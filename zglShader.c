...
_ZGL afxError _DpuSyncShd(sglDpu* dpu, avxShader shd, avxShaderStage stage, glVmt const* gl)
{
    //AfxEntry("shd=%p", shd);
...
_ZGL afxError _SglShdDtor(avxShader shd)
{
    ...
    afxDrawContext dctx = AfxGetParent(shd);
...
_ZGL afxError _SglShdCtor(avxShader shd, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &shd, afxFcc_SHD);
